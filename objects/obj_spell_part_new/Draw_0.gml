...
if (surface_exists(clockwise_surface) and surface_exists(anticlockwise_surface)) {
	...
	var _x, _y;
	_x = x - lengthdir_x(size*2, -90 + image_angle)*(type == TYPE.CONVERTER)
	_y = y - lengthdir_y(size*2, -90 + image_angle)*(type == TYPE.CONVERTER)
	...
	if (type != TYPE.CONVERTER) {
		...
		draw_text_circle_spaced(_x, _y, name, size - 30, 180, -_dir)
		draw_circle_outline(_x, _y, size - 40)
		draw_circle_outline(_x, _y, size - 20)
			
		//input text
		draw_set_colour(COLOUR.SURFACE_EMPTY)
		draw_polygon(_x, _y, size/cos(degtorad(180/children_number)), _dir + zero_angle + 180/children_number, children_number, true)
		draw_set_colour(image_blend)
		draw_polygon(_x, _y, size/cos(degtorad(180/children_number)), _dir + zero_angle + 180/children_number, children_number, false)
		draw_circle_outline(_x, _y, size)
		for (var i = 0; i < input_number; i++) {
			draw_set_colour(input_colour[i])
			draw_text_circle(_x, _y, inputs[i] + "   ", size - 10, 180, -(_dir + zero_angle + (360/children_number)*i - (children_number-2)*180/children_number), 360/children_number, true)
		}
		
		//draw the sprite
		gpu_set_texfilter(false)
		draw_sprite_ext(sprite_index, 0, _x, _y, 1, 1, image_angle - 180, image_blend, 1)
		_dir -= 30
		_x += x - _mid
		_y += y - _mid
	}
	...
	for (var i = 0; i < array_length_1d(connector_queue); i++) {
		with (connector_queue[i]) {
			draw_connector(_x, _y, x, y, name, image_blend, size*image_xscale*(1 + 3*(children_number > 0)), other.size*other.image_xscale, _dir, 1/(1+other.level*0.5))
		}
	}
	...
	switch (type) {
		...
		case TYPE.CONVERTER:
			...
			var _x, _y;
			_x = x - lengthdir_x(size*2, 90)
			_y = y - lengthdir_y(size*2, 90)
			...
			for (i = 0; i < children_number; i++) {
				with (children[i]) {
					if (children_number = 0) {
						...
						get_rel_pos_new(other.id, i)
						
						surface_set_target(other.anticlockwise_surface);
						draw_set_colour(COLOUR.EMPTY)
						draw_polygon(x, y, size/_l, _d + 180*(i == 0), other.children_number, true)
						draw_set_colour(other.image_blend)
						draw_polygon(x, y, size/_l, _d + 180*(i == 0), other.children_number, false)
						surface_reset_target()
					} else {
						event_perform(ev_draw, 0)	
					}
