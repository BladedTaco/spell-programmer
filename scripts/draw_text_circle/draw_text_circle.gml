///@func draw_text_circle(x, y, str, radius, initdir, arc*, spaced*, centered*)
...
if (argument_count > 5) {
	...
	//check for spaced
	if (argument_count > 6) {
		_str = string_repeat(_str, max(floor(2*pi*abs(argument[3])/string_width(_str))*_arc*argument[6], 1))
		//check for centered
		if (argument_count > 7) {
			_spaced = argument[6]
			if (argument[6]) {
